...
public class Context implements OptionHandler {
    ...
    public @NotNull Optional<Option> parse(@NotNull CommandSet parentSet,
        ...
        if (section.contains(path + ".command-execution")) {
            typeMap.put(FilterContextType.COMMAND_EXECUTION,
                section.getBoolean(path + ".command-execution"));
        } else {
            for (CommandSetPreset preset : parentSet.presets()) {
                if (preset.section().contains(path + ".command-execution")) {
                    typeMap.put(FilterContextType.COMMAND_EXECUTION,
                        preset.section().getBoolean(path + ".command-execution"));
                    break;
                }
            }
        }
        ...
        if (section.contains(path + ".command-suggestion")) {
            typeMap.put(FilterContextType.COMMAND_SUGGESTION,
                section.getBoolean(path + ".command-suggestion"));
        } else {
            for (CommandSetPreset preset : parentSet.presets()) {
                if (preset.section().contains(path + ".command-suggestion")) {
                    typeMap.put(FilterContextType.COMMAND_SUGGESTION,
                        preset.section().getBoolean(path + ".command-suggestion"));
                    break;
                }
            }
        }
